...
export class PseudoState extends Vertex {
  ...
  public constructor(
    name: string,
    parent: State | Region,
    public readonly kind: PseudoStateKind = PseudoStateKind.Initial
  ) {
    super(name, parent);
  }
  ...
  public isHistory(): boolean {
    return (
      this.kind === PseudoStateKind.DeepHistory ||
...
export class State extends Vertex {
   ...
   */
  public entry<T extends Action>(entryAction: T) {
...
export class Transition {
  ...
  public constructor(
    ...
    public readonly kind: TransitionKind = TransitionKind.External
  ) {
    if (!this.target) {
  ...
  public toString(): string {
    return `[${
      this.target ? this.source + ' -> ' + this.target : this.source
    }]`;
  }
}
...
export class JSONInstance implements IInstance {
  ...
  public getLastKnownState(region: Region): State | undefined {
    ...
    return region.vertices.reduce<State | undefined>(
      (result, item) =>
        item instanceof State && item.name === lastKnown ? item : result,
      undefined
    );
  }
  ...
  private getNode(stateOrRegion: State | Region): IJSONNode {
    if (stateOrRegion.parent) {
      ...
      let node = parentNode.children.reduce(
        (result, item) => (item.name === stateOrRegion.name ? item : result),
        undefined
      );
 ...
 */
/** @internal */ async function invoke(
...
export async function initialise(
  model: StateMachine,
  instance?: IInstance,
  autoInitialiseModel: boolean = true
): Promise<void> {
  if (instance) {
...
export async function evaluate(
  ...
  return await evaluateState(model, instance, message);
}
 ...
 * @returns Returns true if the message caused a [[Transition]].
 */
/** @internal */ async function evaluateState(
  state: State,
  instance: IInstance,
  message: any
): Promise<boolean> {
  ...
  if (message !== state) {
    ...
    for (let region in state.regions) {
      ...
      if (
        currentState &&
        (await evaluateState(currentState, instance, message))
      ) {
        result = true;
  ...
  if (result) {
    if (message !== state && isComplete(state, instance)) {
      await evaluateState(state, instance, state);
    }
  } else {
    // otherwise look for a transition from this state
    const transitions = state.outgoing.filter(transition =>
      transition.guard(message, instance)
    );
 ...
 */
/** @internal */ async function traverse(
  ...
  await invoke(onTraverse, message, instance);
  ...
  if (transition.target) {
    ...
    if (
      ...
      await traverse(
        selectTransition(transition.target, instance, message),
        instance,
        message
      );
    }
    ...
    // test for completion transitions
      ...
      transition.target instanceof State &&
      isComplete(transition.target, instance)
    ) {
      await evaluateState(transition.target, instance, transition.target);
    }
  }
 ...
 */
/** @internal */ function selectTransition(
  pseudoState: PseudoState,
  instance: IInstance,
  message: any
): Transition {
  const transitions = pseudoState.outgoing.filter(transition =>
    transition.guard(message, instance)
  );

  if (pseudoState.kind === PseudoStateKind.Choice) {
    return transitions.length !== 0
      ? transitions[random(transitions.length)]
...
/** @internal */ function findElse(pseudoState: PseudoState): Transition {
  return pseudoState.outgoing.filter(
    transition => transition.guard === FalseFunc
  )[0];
}
...
/** @internal */ class InitialiseTransitions extends Visitor<
  (vertexOrRegion: Vertex | Region) => Behavior
> {
  async visitTransition(
    transition: Transition,
    behavior: (vertexOrRegion: Vertex | Region) => Behavior
  ) {
    ...
    switch (transition.kind) {
      ...
      case TransitionKind.Local:
        await this.visitLocalTransition(transition, behavior);
        break;
  ...
  visitInternalTransition(
    ...
    if (internalTransitionsTriggerCompletion) {
      transition.onTraverse.push(async (message, instance) => {
        if (
          transition.source instanceof State &&
          isComplete(transition.source, instance)
        ) {
          await evaluateState(transition.source, instance, transition.source);
        }
      });
    }
  }
  ...
  async visitLocalTransition(
    ...
    transition.onTraverse.push(async (message, instance) => {
      ...
      if (currentState) {
        await invoke(behavior(currentState).leave, message, instance);
      }

      // perform the transition action;
      await invoke(transition.transitionBehavior, message, instance);

      // enter the target ancestry
      while (i < targetAncestors.length) {
        this.cascadeElementEntry(
          transition,
          behavior,
          targetAncestors[i++],
          targetAncestors[i],
          async behavior => await invoke(behavior, message, instance)
        );
      }

      // trigger cascade
      await invoke(behavior(transition.target!).endEnter, message, instance);
    });
  }

  // initialise external transitions: these are abritarily complex
  visitExternalTransition(
    transition: Transition,
    behavior: (vertexOrRegion: Vertex | Region) => Behavior
  ) {
    ...
    push(
      transition.onTraverse,
      behavior(sourceAncestors[i]).leave,
    ...
    while (i < targetAncestors.length) {
      this.cascadeElementEntry(
        ...
        targetAncestors[i++],
        targetAncestors[i],
        behavior => push(transition.onTraverse, behavior)
      );
    }
  ...
  cascadeElementEntry(
    ...
    vertex: Vertex,
    next: Vertex,
    task: (behavior: Array<Action>) => void
  ) {
...
/** @internal */ class InitialiseElements extends Visitor<boolean> {
  ...
  /** @internal */ behavior(namedElement: Vertex | Region): Behavior {
    return (
      this.behaviors[namedElement.toString()] ||
  ...
  visitRegion(region: Region, deepHistoryAbove: boolean) {
    const regionInitial = region.vertices.reduce<PseudoState | undefined>(
      (result, vertex) =>
        vertex instanceof PseudoState && vertex.isInitial() ? vertex : result,
      undefined
    );
    ...
    this.behavior(region).leave.push(async (message, instance) => {
      const currentState = instance.getCurrent(region); // NOTE: fix for Joel bug

      if (currentState) {
        await invoke(this.behavior(currentState).leave, message, instance);
      }
    });
    ...
    if (deepHistoryAbove || !regionInitial || regionInitial.isHistory()) {
      ...
      this.behavior(region).endEnter.push(
        async (message, instance, deepHistory) =>
          await invoke(
            this.behavior(
              deepHistory || regionInitial!.isHistory()
                ? instance.getLastKnownState(region) || regionInitial!
                : regionInitial!
            ).enter(),
            message,
            instance,
            deepHistory || regionInitial!.kind === PseudoStateKind.DeepHistory
          )
      );
    } else {
      // TODO: validate initial region
      push(
        this.behavior(region).endEnter,
        this.behavior(regionInitial).enter()
      );
    }
  ...
  visitVertex(vertex: Vertex, deepHistoryAbove: boolean) {
    ...
    this.behavior(vertex).beginEnter.push(async (message, instance) => {
      if (vertex.parent) {
  ...
  visitPseudoState(pseudoState: PseudoState, deepHistoryAbove: boolean) {
    ...
    if (pseudoState.isInitial()) {
      this.behavior(pseudoState).endEnter.push(
        async (message, instance, deepHistory) => {
          let currentState: State | undefined;

          if (
            (deepHistory || pseudoState.isHistory()) &&
            (currentState = instance.getLastKnownState(pseudoState.parent))
          ) {
            await invoke(this.behavior(pseudoState).leave, message, instance);
            await invoke(
              this.behavior(currentState).enter(),
              message,
              instance,
              deepHistory || pseudoState.kind === PseudoStateKind.DeepHistory
            );
          } else {
            await traverse(pseudoState.outgoing[0], instance);
          }
        }
      );
      ...
      this.behavior(pseudoState).beginEnter.push(async (message, instance) => {
  ...
  visitStateMachine(stateMachine: StateMachine, deepHistoryAbove: boolean) {
    ...
    stateMachine.accept(
      new InitialiseTransitions(),
      (vertexOrRegion: Vertex | Region): Behavior =>
        this.behavior(vertexOrRegion)
    );
...
/** @internal */ class Validator extends Visitor<string> {
  ...
  public visitPseudoState(pseudoState: PseudoState): void {
    ...
    if (
      pseudoState.kind === PseudoStateKind.Choice ||
      ...
      if (pseudoState.outgoing.length === 0) {
        console.error(
          `${pseudoState}: ${
            pseudoState.kind
          } pseudo states must have at least one outgoing transition.`
        );
      }

      // choice and junction pseudo state can have at most one else transition
      if (
        pseudoState.outgoing.filter((transition: Transition) => {
        ...
        console.error(
          `${pseudoState}: ${
            pseudoState.kind
          } pseudo states cannot have more than one Else transitions.`
        );
      }
    } else {
      ...
      if (
        ...
        console.error(
          `${pseudoState}: ${
            pseudoState.kind
          } pseudo states cannot have Else transitions.`
        );
      }
      ...
      if (pseudoState.isInitial()) {
        if (pseudoState.outgoing.length > 1) {
          ...
          console.error(
            `${pseudoState}: initial pseudo states must have no more than one outgoing transition.`
          );
          ...
          if (pseudoState.outgoing[0].guard !== TrueFunc) {
            console.error(
              `${pseudoState}: initial pseudo states cannot have a guard condition.`
            );
          }
        }
      }
  ...
  public visitRegion(region: Region): void {
    ...
    if (initial > 1) {
      console.error(
        `${region}: regions may have at most one initial pseudo state.`
      );
    }

    if (deepHistory > 1) {
      console.error(
        `${region}: regions may have at most one deep history pseudo state.`
      );
    }

    if (shallowHistory > 1) {
      console.error(
        `${region}: regions may have at most one shallow history pseudo state.`
      );
    }
  }
  public visitState(state: State): void {
    ...
    if (
      state.regions.filter(region => region.name === Region.defaultName)
        .length > 1
    ) {
      console.error(
        `${state}: a state cannot have more than one region named ${
          Region.defaultName
        }`
      );
    }
  ...
  public visitFinalState(finalState: FinalState): void {
    ...
    if (finalState.outgoing.length !== 0) {
      console.error(
        `${finalState}: final states must not have outgoing transitions.`
      );
    }
  ...
  public visitTransition(transition: Transition): void {
    ...
    if (transition.kind === TransitionKind.Local) {
      if (
        ...
        console.error(
          `${transition}: local transition target vertices must be a child of the source composite sate.`
        );
      }
    }
  }
